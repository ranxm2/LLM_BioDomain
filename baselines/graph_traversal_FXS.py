...
def main(
    ...
    random.seed(seed)
    AD_BioDomains = list(FXS_BioDomain_GOID_map.keys())
    FXS_BioDomain_GOID_map_reverse = {
        v: k for k, v in FXS_BioDomain_GOID_map.items() if v is not None
    }
    ...
    for row in tqdm(df_dicts):
        ...
        if row['GO_ID'] in graph:
            ...
            # set score as 1.0
            preds = {FXS_BioDomain_GOID_map_reverse[d]: 1.0 for d in descendants if d in FXS_BioDomain_GOID_map_reverse}
            if len(preds) > 0:
                hit_traversal += 1
        else:
            preds = {}
