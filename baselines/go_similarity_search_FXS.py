...
from .meta_data import FXS_Biodomain_GO_map 



# # test
# FXS_BioDomain_GOID_map = {
#     'Synapse': 'GO:0045202',
...
def main_expand(
    ...
    go_to_domain = {go: bd for bd, go in FXS_Biodomain_GO_map.items() if go}
    ...
    domain_members = {}
    for domain, seed_go in FXS_Biodomain_GO_map.items():
        if seed_go is None:
    ...
    for rec in df_true:
        ...
        remaining = [d for d in FXS_Biodomain_GO_map if d not in preds]
        k = topk - len(preds)
        if k > 0:
