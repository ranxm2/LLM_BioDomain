...
BIODOMAIN_LIST = [
    ...
    'Unknown'  # Added for cases where no domain is applicable
]
...
Below is a GO term with its definition and its position in the ontology hierarchy. From the list of Biodomains, select the top five labels that best reflect this term’s relevance to Fragile X Syndrome, ordered from most to least appropriate.  Provide exactly five domain names only, separated by commas, with no numbering, bullets, or additional text. Use 'Unknown' when you are not sure.
