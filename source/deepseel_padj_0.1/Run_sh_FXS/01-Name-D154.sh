#!/bin/bash
#SBATCH --job-name=BD_D154
#SBATCH --output=../logs_FXS/D154/Name-%A_%a_out.txt
#SBATCH --error=../logs_FXS/D154/Name-%A_%a_err.txt
#SBATCH --array=1-40
#SBATCH --ntasks=1
...
python ./source/deepseek/deepseek_biodomain_name_temp_v2.py \
  ...
  --df_go_path data/D154-GSVA-padj-0.1-node.csv \
  --obo_path data/go-basic.obo \
  --similarity_path data/go_jaccard_long_filtered.csv \
  --result_dir Experiment/DeepSeek/03-FXS-padj-0.1/D154/Temp_${TEMP} \
