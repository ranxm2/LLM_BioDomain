#!/bin/bash
#SBATCH --job-name=BD_D56
#SBATCH --output=../logs_FXS/D56/Name-%A_%a_out.txt
#SBATCH --error=../logs_FXS/D56/Name-%A_%a_err.txt
#SBATCH --array=1-40
#SBATCH --ntasks=1
...
python ./source/deepseek/deepseek_biodomain_name_temp_v2.py \
  ...
  --df_go_path data/D56-GSVA-padj-0.1-node.csv \
  --obo_path data/go-basic.obo \
  --similarity_path data/go_jaccard_long_filtered.csv \
  --result_dir Experiment/DeepSeek/03-FXS-padj-0.1/D56/Temp_${TEMP} \
